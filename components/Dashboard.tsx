...
import { saveRoadmap, saveUser, getRoadmap, getCareerData, saveCareerData, setCurrentUser, getNewsCache, saveNewsCache, getDailyQuizCache, saveDailyQuizCache, deleteUser } from '../services/store';
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  useEffect(() => {
      ...
      if (user.theme && user.theme !== 'indigo') { // Indigo is default/base
  ...
  // Chat Context Effect
  useEffect(() => {
      // Add initial greeting or context switch message
      ...
      setChatHistory(prev => {
          ...
          // If switching careers, add a system note
          return [...prev, {
              id: Date.now().toString(),
              role: 'bot',
  ...
  // Load stats for all careers when entering Career Tab
  useEffect(() => {
      if (activeTab === 'career') {
  ...
  const loadNews = async (query?: string) => {
      ...
      // Try local cache first only if general query
      if (!query) {
  ...
  // Load News on mount or career change
  useEffect(() => {
  ...
  // Check Daily Quiz Logic (Cache First)
  useEffect(() => {
    const checkDailyQuiz = async () => {
        ...
        // Try getting cached pending quiz for today
        ...
        try {
            ...
            if (quiz) {
                setDailyQuiz(quiz);
                saveDailyQuizCache(user.id, career.id, quiz); // Save to cache
                setQuizState('active');
            } else {
                setQuizState('already_done'); // Fallback
            }
        } catch (e) {
            setQuizState('already_done');
        }
  ...
  const handleQuizAnswer = (index: number) => {
      ...
      if (isRight) {
          ...
          if (user.lastQuizDate) {
              ...
              if (user.lastQuizDate === yesterdayStr) {
                  newStreak += 1;
              } else {
                  newStreak = 1; 
              }
  ...
  const handlePracticeSearch = async () => {
      ...
      if (practiceTab === 'quiz') {
          ...
          // Pass the company filter or custom params if in AI Challenge mode
  ...
  // Trigger search when tab changes or filter changes (except for AI Challenge which is manual)
  useEffect(() => {
      if (activeTab === 'practice') {
  ...
  const handleDeleteCareerRequest = (careerId: string) => {
  ...
  const executeDeleteCareer = () => {
      ...
      if (updatedActiveCareers.length === 0) {
          ...
          if (careerId === user.currentCareerId && nextCareerId) {
  ...
  // Filtered Interview Questions Logic (Unified or Specific)
  // Logic moved to handlePracticeSearch to fetch correct set
          ...
          <div className="bg-slate-900 border border-emerald-500/30 rounded-3xl p-8 max-w-lg w-full text-center relative overflow-hidden shadow-2xl">
              ...
              <p className="text-slate-300 mb-8">
                  {pacing.status === 'behind' 
                    ? "You've caught up a bit, but still behind schedule. Adapt?" 
                    : pacing.status === 'ahead' 
                        ? "You are crushing it! Want to speed up or go deeper?"
                        : "Great pace! How would you like to proceed?"}
              </p>
              ...
              {isAdapting ? (
                  ...
                  <div className="space-y-3 text-left">
                      {/* BEHIND SCHEDULE OPTIONS */}
                      {pacing.status === 'behind' && (
                          <>
                            <button 
                                onClick={() => handleAdaptation('maintain_pressure')}
                            ...
                            >
                                <div>
                                    <div className="font-bold text-white mb-1">Maintain Timeline</div>
                                    <div className="text-xs text-slate-400">Keep target date. Rearrange tasks to fit.</div>
                                </div>
                                <ArrowRight className="h-5 w-5 text-slate-500 group-hover:text-blue-400" />
                            </button>

                            <button 
                                onClick={() => handleAdaptation('reduce_difficulty')}
                            ...
                            >
                                <div>
                                    <div className="font-bold text-white mb-1">Reduce Difficulty</div>
                                    <div className="text-xs text-slate-400">Simplify remaining tasks to catch up.</div>
                                </div>
                                <ArrowRight className="h-5 w-5 text-slate-500 group-hover:text-emerald-400" />
                            </button>

                            <button 
                                onClick={() => handleAdaptation('simplify_schedule')} 
                            ...
                            >
                                <div>
                                    <div className="font-bold text-white mb-1">Prioritize Basics</div>
                                    <div className="text-xs text-slate-400">Cut non-essentials to finish on time.</div>
                                </div>
                      ...
                      {pacing.status !== 'behind' && (
                          <>
                            ...
                            >
                                <div>
                                    <div className="font-bold text-white mb-1">Change Pace (Relax)</div>
                                    <div className="text-xs text-slate-400">Keep date. Spread tasks out more.</div>
                                </div>
                                <ArrowRight className="h-5 w-5 text-slate-500 group-hover:text-blue-400" />
                            </button>
                            ...
                            >
                                <div>
                                    ...
                                    <div className="text-xs text-slate-400">Keep date. Add advanced challenges.</div>
                                </div>
                                <ArrowRight className="h-5 w-5 text-slate-500 group-hover:text-purple-400" />
                            </button>
                            ...
                            >
                                <div>
                                    ...
                                    <div className="text-xs text-slate-400">Update target date to now.</div>
                                </div>
                                <ArrowRight className="h-5 w-5 text-slate-500 group-hover:text-emerald-400" />
                            </button>
                          </>
  ...
  const FeedbackModal = () => (
      <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-md w-full relative shadow-2xl">
              ...
              <button 
                  onClick={() => {
                      showToastMsg("Feedback sent! Nova thanks you.");
                      setShowFeedbackModal(false);
                      setFeedbackText('');
                  }}
                  disabled={!feedbackText.trim()}
                  className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl transition-all disabled:opacity-50"
              >
                  Send Feedback
              </button>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 max-w-lg w-full text-center shadow-2xl">
              ...
              <p className="text-slate-300 mb-6">
                  {dateStrategyType === 'extension' 
                      ? "You have more time. How should Nova adapt?" 
                      : "You have less time. How should Nova adapt?"}
              </p>
              ...
              {isAdapting ? (
                  ...
                  <div className="space-y-4">
                      {dateStrategyType === 'extension' ? (
                          ...
                          <>
                              <button 
                                  onClick={() => handleAdaptation('compress_schedule', pendingTargetDate)} // High Pressure
                              ...
                              >
                                  <div>
                                      <div className="font-bold text-white mb-1">Increase Pressure</div>
                                      <div className="text-xs text-slate-400">Compress tasks to fit. Harder pace.</div>
                                  </div>
                              ...
                              >
                                  <div>
                                      <div className="font-bold text-white mb-1">Adapt (Minimize Difficulty)</div>
                                      <div className="text-xs text-slate-400">Remove/simplify topics to maintain pace.</div>
                                  </div>
                                  <ArrowRight className="h-5 w-5 text-slate-500 group-hover:text-blue-400" />
                              </button>
                          </>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-slate-900 border border-red-500/50 rounded-3xl p-8 max-w-sm w-full text-center relative shadow-2xl">
                  ...
                  <p className="text-slate-400 text-sm mb-6">
                      {type === 'reset_all' 
                        ? "This will wipe all progress for all careers. This action cannot be undone."
                        : "This will permanently delete your account and all data. This action cannot be undone."}
                  </p>
                  
                  <div className="mb-6 text-left">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">
                          Type <span className="text-white select-all">{requiredText}</span> to confirm
                      </label>
                      <input 
                        type="text" 
                        className="w-full p-4 rounded-xl bg-slate-950 border border-slate-800 text-white focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none uppercase tracking-widest font-bold text-center"
                        value={inputValue}
                        onChange={e => setConfirmAction({...confirmAction, inputValue: e.target.value.toUpperCase()})}
                  ...
                  <div className="flex gap-3">
                      ...
                      <button 
                        onClick={type === 'reset_all' ? executeResetAll : executeDeleteAccount}
                        disabled={!isMatch}
                        className="flex-1 py-3 bg-red-600 hover:bg-red-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all"
                      >
                          {type === 'reset_all' ? 'Reset All' : 'Delete Account'}
                      </button>
                  </div>
              </div>
          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-slate-900 border border-red-500/30 rounded-3xl p-6 max-w-sm w-full relative shadow-2xl">
                  <div className="flex items-start gap-4 mb-4">
                      ...
                      <div>
                          ...
                          <p className="text-slate-400 text-sm mt-1">
                              Are you sure you want to remove <span className="text-white font-medium">"{careerTitle}"</span>? All progress and roadmaps for this path will be lost.
                          </p>
                      </div>
                  </div>
                  ...
                  <div className="flex gap-3 mt-4">
                      ...
                      <button 
                          onClick={executeDeleteCareer}
                          className="flex-1 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl shadow-lg shadow-red-900/20 transition-all text-sm flex items-center justify-center gap-2"
                      >
                          Delete
                      </button>
                  </div>
          ...
          <div className="space-y-8 animate-fade-in pb-10">
            ...
            <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
              <div>
                ...
                <p className="text-slate-400 mt-1">Welcome back, <span className="text-white font-medium">{user.username}</span>.</p>
              </div>
            ...
            {/* Daily Quiz & Gamification Section */}
                 ...
                 <div className="md:col-span-2 bg-gradient-to-br from-indigo-900/40 to-slate-900 p-6 rounded-3xl border border-indigo-500/30 relative overflow-hidden shadow-lg shadow-indigo-900/10 min-h-[300px] flex flex-col justify-center">
                     {showConfetti && (
                         <div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center">
                             <div className="animate-ping w-32 h-32 bg-emerald-500/20 rounded-full absolute"></div>
                         </div>
                     )}
                     ...
                     <div className="flex items-center justify-between mb-6">
                         <div className="flex items-center gap-2 text-indigo-400">
                             ...
                             <span className="font-bold uppercase text-xs tracking-wider">Daily Challenge</span>
                         </div>
                         <div className="flex items-center gap-3">
                             <div className="flex items-center gap-1.5 bg-slate-900/50 px-3 py-1.5 rounded-lg border border-slate-700">
                                 <Flame className={`h-4 w-4 ${user.streak > 0 ? 'text-orange-500 fill-orange-500' : 'text-slate-500'}`} />
                     ...
                     {quizState === 'already_done' && (
                         <div className="text-center py-8">
                             <div className="w-16 h-16 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                                 <CheckCircle2 className="h-8 w-8 text-emerald-400" />
                             </div>
                             <h3 className="text-xl font-bold text-white mb-2">You're all set for today!</h3>
                             <p className="text-slate-400 mb-6">Come back tomorrow to keep your {user.streak} day streak alive.</p>
                             <div className="inline-block bg-slate-800 px-4 py-2 rounded-xl text-sm text-slate-300">
                                 Next quiz available in <span className="text-white font-mono">24h</span>
                             </div>
                         </div>
                     )}
                         ...
                         <div className="text-center py-6 animate-fade-in">
                             {isQuizCorrect ? (
                                 <>
                                    ...
                                    <h3 className="text-2xl font-bold text-white mb-2">Correct! +10 XP</h3>
                                    <p className="text-slate-300 mb-6">{dailyQuiz.explanation}</p>
                                    <div className="w-full bg-slate-800/50 p-4 rounded-xl text-left border border-slate-700">
                                        <div className="text-xs text-slate-500 uppercase font-bold mb-1">Answer Summary</div>
                                        <div className="text-white">{dailyQuiz.options[dailyQuiz.correctIndex]}</div>
                                    </div>
                                 </>
                             ) : (
                                 <>
                                    <div className="text-4xl mb-4">💪</div>
                                    <h3 className="text-2xl font-bold text-white mb-2">Not quite. Keep learning!</h3>
                                    <p className="text-slate-300 mb-6">Streak reset to 0. Don't give up!</p>
                                    <div className="w-full bg-slate-800/50 p-4 rounded-xl text-left border border-slate-700">
                                        <div className="text-xs text-slate-500 uppercase font-bold mb-1">Correct Answer</div>
                                        <div className="text-emerald-400 font-bold mb-2">{dailyQuiz.options[dailyQuiz.correctIndex]}</div>
                                        <div className="text-sm text-slate-300">{dailyQuiz.explanation}</div>
                                    </div>
                                 </>
                             )}
                         </div>
                                     ...
                                     <button
                                        ...
                                        className={`w-full text-left p-4 rounded-xl border transition-all ${
                                            selectedQuizOption !== null 
                                                ? i === dailyQuiz.correctIndex 
                                                    ? 'bg-emerald-500/20 border-emerald-500 text-white'
                                                    : i === selectedQuizOption
                                                        ? 'bg-red-500/20 border-red-500 text-white'
                                                        : 'bg-slate-900 border-slate-800 text-slate-500 opacity-50'
                                                : 'bg-slate-900/50 border-slate-700 text-slate-200 hover:bg-indigo-900/30 hover:border-indigo-500 hover:text-white'
                                        }`}
                                     >
                                         <div className="flex items-center gap-3">
                                             <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs font-bold ${
                                                 selectedQuizOption !== null && i === dailyQuiz.correctIndex 
                 ...
                 <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex flex-col justify-between">
                     <div className="mb-6">
                         <h3 className="text-slate-400 font-medium mb-4 flex items-center gap-2">
                             <Target className="h-4 w-4 text-indigo-400" /> Career Progress
                         </h3>
                     ...
                     <div className="space-y-4">
                         <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex items-center justify-between">
                             <div>
                                 <div className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Time Left</div>
                         ...
                         <div className={`p-4 rounded-2xl border flex items-center justify-between ${
                             pacing.status === 'ahead' ? 'bg-emerald-500/10 border-emerald-500/20' : 
                             pacing.status === 'behind' ? 'bg-red-500/10 border-red-500/20' : 
                             'bg-blue-500/10 border-blue-500/20'
                         }`}>
                             <div>
                                 <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${
                                     pacing.status === 'ahead' ? 'text-emerald-400' : 
                                     pacing.status === 'behind' ? 'text-red-400' : 
                                     'text-blue-400'
                                 }`}>Current Pace</div>
                                 <div className="text-sm font-bold text-white">{pacing.message}</div>
                             </div>
                             {pacing.status === 'ahead' ? <TrendingUp className="h-5 w-5 text-emerald-400" /> : <TrendingUp className="h-5 w-5 text-slate-400" />}
                         </div>
                     </div>
                 </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                 ...
                 <div className="space-y-1">
                     {isNewsLoading ? (
                         [1,2,3].map(i => (
                             <div key={i} className="h-16 bg-slate-800/50 rounded-xl animate-pulse my-2"></div>
                         ))
                     ) : news.length === 0 ? (
                         <div className="text-slate-500 text-center py-8">No recent headlines found.</div>
                     ) : (
                         news.map((item, i) => (
                             <a 
                                 key={i} 
                             ...
                             >
                                 <div className="flex items-center gap-4">
                                     <span className="text-xs font-bold text-slate-500 w-24 truncate text-right">{item.source}</span>
                                     <h3 className="text-sm md:text-base font-medium text-slate-300 group-hover:text-white transition-colors line-clamp-1">
                                         {item.title}
                                     </h3>
                                 </div>
                                 <ExternalLink className="h-4 w-4 text-slate-600 group-hover:text-indigo-400 transition-colors shrink-0" />
                             </a>
                         ))
                     )}
                 </div>
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
                           <div>
                               <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                                   <GraduationCap className="h-6 w-6 text-indigo-500" /> Practice Arena
                               </h2>
                               <p className="text-slate-400 text-sm mt-1">Master your skills in {career.title}</p>
                           </div>
                           ...
                           <div className="relative group w-full md:w-80">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all text-sm"
                                    value={practiceSearch}
                                    onChange={e => setPracticeSearch(e.target.value)}
                                    onKeyDown={e => e.key === 'Enter' && handlePracticeSearch()}
                                />
                           </div>
                      </div>

                      <div className="flex gap-2 p-1 bg-slate-900 rounded-xl border border-slate-800 w-full md:w-auto self-start inline-flex">
                          {[
                              { id: 'quiz', icon: BrainCircuit, label: 'Practice Questions' },
                              { id: 'interview', icon: MessageSquare, label: 'Question Bank' },
                              { id: 'simulation', icon: PlayCircle, label: 'Simulation' },
                          ].map(tab => (
                              <button
                                key={tab.id}
                                onClick={() => setPracticeTab(tab.id as any)}
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${practiceTab === tab.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}
                              >
                  ...
                  <div className="p-6 md:p-8 flex-1 bg-slate-900">
                      {isPracticeLoading ? (
                           <div className="flex flex-col items-center justify-center py-20">
                               ...
                               <p className="text-slate-400 animate-pulse">Nova is generating your practice session...</p>
                           </div>
                      ) : (
                          <>
                              {/* QUIZ MODE */}
                              {practiceTab === 'quiz' && (
                                  <div className="animate-fade-in space-y-8">
                                      {/* Topics Filter */}
                                      <div>
                                          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Suggested Topics</h3>
                                          <div className="flex flex-wrap gap-2">
                                              <button 
                                                onClick={() => setSelectedTopic(null)}
                                                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${!selectedTopic ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-600'}`}
                                              >
                                                  General
                                              </button>
                                              {practiceTopics.map(t => (
                                                  <button 
                                                    ...
                                                    className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${selectedTopic === t ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-600'}`}
                                                  >
                                                      {t}
                                                  </button>
                                              ))}
                                      ...
                                      <div className="space-y-6">
                                          {practiceQuestions.length > 0 ? (
                                              practiceQuestions.map((q, qIdx) => (
                                                  <div key={q.id || qIdx} className="bg-slate-950 border border-slate-800 rounded-2xl p-6">
                                              ...
                                              <div className="text-center py-10 text-slate-500">
                                                  No questions generated yet. Try searching or selecting a topic!
                                              </div>
                                          )}
                                      </div>
                                  </div>
                              ...
                              {/* INTERVIEW MODE (Company Question Bank) */}
                              {practiceTab === 'interview' && (
                                  <div className="animate-fade-in space-y-8">
                                      {/* Company Chips Filter */}
                                      <div>
                                          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Filter by Company / Mode</h3>
                                          <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">
                                              {['All', 'Google', 'Amazon', 'Microsoft', 'Netflix', 'Meta', 'Startups', 'Aptitude', 'AI Challenge'].map(mode => (
                                                  <button
                                                      key={mode}
                                                      onClick={() => setCompanyFilter(mode)}
                                                  ...
                                                  >
                                                      {mode === 'AI Challenge' && <Sparkles className="h-3 w-3" />}
                                                      {mode === 'Aptitude' && <Cpu className="h-3 w-3" />}
                                                      {mode}
                                                  </button>
                                              ))}
                                          </div>
                                      ...
                                      {companyFilter === 'AI Challenge' && (
                                          <div className="bg-slate-950 border border-fuchsia-500/30 rounded-2xl p-6 animate-fade-in relative overflow-hidden">
                                              ...
                                              <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                                                  <Sparkles className="h-5 w-5 text-fuchsia-500" /> Custom AI Generator
                                              </h3>
                                              <div className="grid md:grid-cols-3 gap-4 mb-4">
                                                  <div className="md:col-span-2">
                                                      <label className="text-xs text-slate-500 block mb-1 uppercase font-bold">Topic</label>
                                                      <input 
                                                          type="text" 
                                                          placeholder={`e.g. Advanced ${career.title} concepts`} 
                                                          className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-fuchsia-500 outline-none"
                                                          value={customGenTopic}
                                                          onChange={e => setCustomGenTopic(e.target.value)}
                                                      />
                                                  </div>
                                                  <div>
                                                      <label className="text-xs text-slate-500 block mb-1 uppercase font-bold">Difficulty</label>
                                                      <select 
                                                          className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-white focus:border-fuchsia-500 outline-none"
                                                          value={customGenDifficulty}
                                                          onChange={e => setCustomGenDifficulty(e.target.value)}
                                                      >
                                                          <option>Easy</option>
                                              ...
                                              <button 
                                                  onClick={handlePracticeSearch}
                                                  className="w-full py-3 bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-fuchsia-900/20"
                                              >
                                                  ...
                                                  <div key={q.id || i} className="bg-slate-950 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between hover:border-indigo-500/50 transition-colors group">
                                                      <div>
                                                          <div className="flex justify-between items-start mb-4">
                                                              <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${
                                                                  q.company?.includes('Aptitude') 
                                                                    ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' 
                                                                    : q.company?.includes('AI Challenge')
                                                      ...
                                                      <div>
                                                          {visibleAnswers.has(q.id) ? (
                                                              <div className="bg-slate-900 p-4 rounded-xl text-sm text-slate-300 border border-slate-800 animate-fade-in">
                                                                  <span className="font-bold text-indigo-400 block mb-1">Answer Guide:</span>
                                                                  {q.answer}
                                                              </div>
                                                          ) : (
                                                              <button 
                                                                onClick={() => toggleAnswerReveal(q.id)}
                                                                className="w-full py-3 border border-slate-800 rounded-xl text-slate-500 hover:text-white hover:bg-slate-800 transition-all text-sm font-medium flex items-center justify-center gap-2"
                                                              >
                                      ...
                                      <div className="bg-slate-950 border border-indigo-500/30 rounded-3xl overflow-hidden shadow-2xl relative">
                                          ...
                                          <div className="p-8">
                                              <div className="flex items-center gap-3 mb-6">
                                                  ...
                                                  <div>
                                                      <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Simulation Arena</h3>
                                                      <h2 className="text-xl font-bold text-white">Scenario Challenge</h2>
                                                  </div>
                                              </div>

                                              <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 mb-8 italic text-slate-300 leading-relaxed">
                                                  ...
                                                  {simulationScenario.options.map((opt, idx) => {
                                                      ...
                                                      return (
                                                          <button
                                                              ...
                                                              disabled={simAnswer !== null}
                                                              className={`w-full text-left p-5 rounded-xl border transition-all relative overflow-hidden ${
                                                                  showResult 
                                                                    ? isCorrect 
                                                                        ? 'bg-emerald-500/20 border-emerald-500 text-white' 
                                                                        : isSelected 
                                                                            ...
                                                                            : 'bg-slate-900 border-slate-800 text-slate-500 opacity-50'
                                                                    : 'bg-slate-900 border-slate-700 text-slate-200 hover:border-indigo-500 hover:bg-slate-800'
                                                              }`}
                                                          >
                                                              {opt}
                                                              {showResult && isCorrect && <CheckCircle2 className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-emerald-400" />}
                                                          </button>
                                                      );
                                                  })}
                                              </div>
                                              ...
                                              {simAnswer !== null && (
                                                  <div className="mt-8 p-6 bg-slate-900 rounded-2xl border border-indigo-500/20 animate-fade-in">
                                                      <h4 className="text-indigo-400 font-bold mb-2 flex items-center gap-2">
                                                          <Zap className="h-4 w-4" /> Nova Analysis
                                                      </h4>
                                                      <p className="text-slate-300">{simulationScenario.explanation}</p>
                                                      <button 
                                                        onClick={handlePracticeSearch}
                                                        className="mt-4 text-sm text-slate-400 hover:text-white underline decoration-slate-600 hover:decoration-white underline-offset-4"
                                                      >
                                                          Next Scenario
                                                      </button>
                                                  </div>
              ...
              <div className="space-y-6 pb-20 animate-fade-in">
                  <div className="flex justify-between items-end">
                      <div>
                          ...
                          <p className="text-slate-400 mt-1">Manage your active career journeys.</p>
                      </div>
                      <button 
                          onClick={() => onAddCareer()}
                          className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-900/20"
                      >
                          <PlusCircle className="h-5 w-5" /> Add Career
                      </button>
                  </div>
                      ...
                      {user.activeCareers.map((c, i) => {
                          ...
                          return (
                              <div key={c.careerId} className={`relative bg-slate-900 border rounded-3xl p-6 transition-all ${isCurrent ? 'border-indigo-500 shadow-xl shadow-indigo-900/10' : 'border-slate-800 hover:border-slate-700'}`}>
                                  {isCurrent && (
                                      <div className="absolute top-0 right-0 bg-indigo-600 text-white text-xs font-bold px-4 py-1.5 rounded-bl-2xl rounded-tr-2xl">
                                          CURRENT FOCUS
                                      </div>
                                  )}
                                  ...
                                  <div className="flex flex-col md:flex-row justify-between gap-6 mb-6">
                                      <div>
                                          ...
                                          <div className="flex flex-wrap gap-3 text-sm text-slate-400">
                                              <span className="flex items-center gap-1.5 bg-slate-950 px-3 py-1 rounded-lg border border-slate-800">
                                                  <CalendarDays className="h-4 w-4 text-indigo-400" /> 
                                                  Start: {new Date(c.addedAt).toLocaleDateString()}
                                              </span>
                                              <span className="flex items-center gap-1.5 bg-slate-950 px-3 py-1 rounded-lg border border-slate-800">
                                                  <Target className="h-4 w-4 text-indigo-400" /> 
                                                  Target: {c.targetCompletionDate}
                                              </span>
                                              <span className="flex items-center gap-1.5 bg-slate-950 px-3 py-1 rounded-lg border border-slate-800 capitalize">
                                      ...
                                      <div className="flex items-center gap-4">
                                          <div className="text-right">
                                              <div className="text-3xl font-bold text-white">{stats.daysLeft}</div>
                                          ...
                                          <div className="text-right">
                                              <div className="text-3xl font-bold text-emerald-400">{stats.progress}%</div>
                                              <div className="text-xs text-slate-500 font-bold uppercase tracking-wider">Complete</div>
                                          </div>
                                      </div>
                                  </div>

                                  <div className="h-3 bg-slate-950 rounded-full overflow-hidden border border-slate-800 mb-6">
                                  ...
                                  <div className="flex gap-4">
                                      {isCurrent ? (
                                          <button 
                                            disabled 
                                            className="flex-1 py-3 bg-slate-800 text-slate-400 font-bold rounded-xl cursor-default"
                                          >
                                            Active
                                          </button>
                                      ) : (
                                      ...
                                      <button 
                                        onClick={() => handleDeleteCareerRequest(c.careerId)}
                                        className="px-4 py-3 bg-slate-900 border border-slate-800 hover:bg-red-900/10 hover:border-red-500/30 text-slate-500 hover:text-red-400 rounded-xl transition-colors"
                                        title="Delete Career"
                                      >
          ...
          return (
              <div className="space-y-8 pb-20 animate-fade-in">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 relative overflow-hidden">
                      ...
                      <div className="relative z-10 flex flex-col md:flex-row items-end gap-6 pt-12">
                          <div className="w-24 h-24 bg-slate-800 rounded-2xl border-4 border-slate-900 shadow-xl flex items-center justify-center text-3xl font-bold text-white">
                              {user.username.charAt(0).toUpperCase()}
                          </div>
                          <div className="flex-1 mb-2">
                              <h2 className="text-3xl font-bold text-white">{user.username}</h2>
                              <div className="flex items-center gap-4 text-slate-400 text-sm mt-1">
                                  <span>Member since {user.joinedAt ? new Date(user.joinedAt).getFullYear() : 2024}</span>
                                  <span>•</span>
                                  <span className="capitalize">{user.subscriptionStatus} Plan</span>
                              </div>
                          </div>
                          <div className="flex gap-2">
                             <div className="bg-slate-950 px-4 py-2 rounded-xl border border-slate-800 flex items-center gap-2">
                                 <Flame className="h-4 w-4 text-orange-500" />
                                 <span className="text-white font-bold">{user.streak} Streak</span>
                             </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                          <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                              <Palette className="h-5 w-5 text-indigo-400" /> Accent Color
                          </h3>
                          
                          <div className="space-y-4">
                              <div className="p-4 bg-slate-950 rounded-xl border border-slate-800">
                                  <div className="text-slate-400 text-sm mb-3">Choose your vibe</div>
                                  <div className="flex flex-wrap gap-3">
                                      {(['indigo', 'emerald', 'rose', 'amber', 'cyan'] as const).map(color => (
                                          <button
                                            key={color}
                                            onClick={() => setAccentColor(color)}
                                            className={`w-10 h-10 rounded-full border-2 transition-all flex items-center justify-center ${user.theme === color ? 'border-white scale-110' : 'border-transparent hover:scale-105'}`}
                                            style={{
                                                backgroundColor: color === 'indigo' ? '#6366f1' : 
                                                                color === 'emerald' ? '#10b981' : 
                                                                color === 'rose' ? '#f43f5e' : 
                                                                color === 'amber' ? '#f59e0b' : '#06b6d4'
                                            }}
                                          >
                                              {user.theme === color && <CheckCircle2 className="h-5 w-5 text-white drop-shadow-md" />}
                                          </button>
                                      ))}
                                  </div>
                              </div>
                              <div className="flex items-center justify-between p-4 bg-slate-950 rounded-xl border border-slate-800">
                                  ...
                                  <span className="text-slate-500 font-mono text-sm">
                                      {user.id.includes('@') ? user.id.replace(/(.{2})(.*)(@.*)/, "$1***$3") : user.id}
                                  </span>
                              </div>
                          </div>
                      </div>
                      ...
                      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
                          <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                              <ShieldAlert className="h-5 w-5 text-red-400" /> Danger Zone
                          </h3>
                          <div className="space-y-4">
                              <button 
                                  onClick={() => setConfirmAction({type: 'reset_all', inputValue: ''})}
                                  className="w-full text-left p-4 bg-slate-950 hover:bg-red-900/10 border border-slate-800 hover:border-red-500/30 rounded-xl text-slate-400 hover:text-red-400 transition-all flex items-center justify-between group"
                              >
                                  <span>Reset All Progress</span>
                                  <RotateCcw className="h-5 w-5 group-hover:rotate-180 transition-transform" />
                              </button>
                              <button 
                                  onClick={() => setConfirmAction({type: 'delete_account', inputValue: ''})}
                                  className="w-full text-left p-4 bg-slate-950 hover:bg-red-900/10 border border-slate-800 hover:border-red-500/30 rounded-xl text-slate-400 hover:text-red-400 transition-all flex items-center justify-between group"
                              >
                  ...
                  <div className="text-center pt-8">
                       ...
                       <div className="mt-8 text-xs text-slate-600">
                            PathFinder AI v1.0.0 • Developed by Hameed Afsar K M
                       </div>
                       
                       <button onClick={() => setShowFeedbackModal(true)} className="mt-4 text-xs text-indigo-500 hover:text-indigo-400">Send Feedback</button>
                  </div>
              </div>
          );
      default:
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-50 pb-20 md:pb-0">
      <main className="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full">
        ...
        {/* FOOTER */}
        <footer className="mt-8 pt-8 border-t border-slate-900 text-center pb-8 md:pb-4">
            <p className="text-slate-600 text-sm">
                Developed by <span className="text-indigo-500 font-semibold"> Hameed Afsar K M</span>
            </p>
        </footer>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-slate-900/90 backdrop-blur-xl border-t border-slate-800 p-2 md:hidden z-40">
        <div className="flex justify-around items-center">
          <button onClick={() => setActiveTab('home')} className={`p-3 rounded-xl transition-all ${activeTab === 'home' ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-500'}`}>
            <Home className="h-6 w-6" />
          </button>
          <button onClick={() => setActiveTab('roadmap')} className={`p-3 rounded-xl transition-all ${activeTab === 'roadmap' ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-500'}`}>
            <Map className="h-6 w-6" />
          </button>
          <button onClick={() => setActiveTab('practice')} className={`p-3 rounded-xl transition-all ${activeTab === 'practice' ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-500'}`}>
            <GraduationCap className="h-6 w-6" />
          </button>
          <button onClick={() => setActiveTab('career')} className={`p-3 rounded-xl transition-all ${activeTab === 'career' ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-500'}`}>
            <Briefcase className="h-6 w-6" />
          </button>
          <button onClick={() => setActiveTab('profile')} className={`p-3 rounded-xl transition-all ${activeTab === 'profile' ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-500'}`}>
      ...
      <nav className="fixed left-0 top-0 h-full w-20 bg-slate-900 border-r border-slate-800 flex-col items-center py-8 hidden md:flex z-50">
        ...
        <div className="flex flex-col gap-4 w-full px-3">
          <button onClick={() => setActiveTab('home')} className={`p-3 rounded-xl transition-all group relative ${activeTab === 'home' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <Home className="h-5 w-5 mx-auto" />
            <span className="absolute left-14 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Home</span>
          </button>
          <button onClick={() => setActiveTab('roadmap')} className={`p-3 rounded-xl transition-all group relative ${activeTab === 'roadmap' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <Map className="h-5 w-5 mx-auto" />
            <span className="absolute left-14 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Roadmap</span>
          </button>
           <button onClick={() => setActiveTab('practice')} className={`p-3 rounded-xl transition-all group relative ${activeTab === 'practice' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <GraduationCap className="h-5 w-5 mx-auto" />
            <span className="absolute left-14 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Practice</span>
          </button>
          <button onClick={() => setActiveTab('career')} className={`p-3 rounded-xl transition-all group relative ${activeTab === 'career' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <Briefcase className="h-5 w-5 mx-auto" />
            <span className="absolute left-14 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Career</span>
          </button>
          <button onClick={() => setActiveTab('profile')} className={`p-3 rounded-xl transition-all group relative ${activeTab === 'profile' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <User className="h-5 w-5 mx-auto" />
            <span className="absolute left-14 bg-slate-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Profile</span>
          </button>
        </div>
      </nav>
      ...
      <button 
        ...
        className="fixed bottom-24 md:bottom-10 right-4 md:right-10 w-14 h-14 bg-indigo-600 hover:bg-indigo-500 rounded-full shadow-2xl shadow-indigo-500/40 flex items-center justify-center z-[60] transition-transform hover:scale-105 active:scale-95"
      >
      ...
      {/* Toast */}
      {toast && (
          <div className="fixed bottom-24 md:bottom-10 left-1/2 -translate-x-1/2 bg-slate-900 border border-emerald-500/50 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-fade-in z-[100]">
