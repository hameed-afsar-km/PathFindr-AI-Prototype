...
const SplashScreen = () => (
  <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center p-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-900/40 via-slate-950 to-slate-950 overflow-hidden">
    ...
    <div className="relative z-10 flex flex-col items-center">
      <div className="mb-8 relative animate-float">
        ...
        <div className="absolute inset-0 animate-[spin_3s_linear_infinite]">
            <div className="absolute top-0 left-1/2 w-2 h-2 bg-indigo-400 rounded-full blur-[1px] shadow-[0_0_10px_rgba(129,140,248,0.8)]"></div>
        </div>
      </div>
    ...
    <div className="absolute bottom-8 flex flex-col items-center opacity-60">
        <p className="text-slate-500 text-[10px] uppercase tracking-[0.2em] mb-1">Architected By</p>
        <p className="text-slate-300 text-xs font-bold tracking-wide">Hameed Afsar K M</p>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const handleOnboardingComplete = async (selectedCareer: CareerOption, eduYear: string, targetDate: string, expLevel: 'beginner' | 'intermediate' | 'advanced', focusAreas: string) => {
    ...
    const updatedUser = { 
        ...
        joinedAt: user.joinedAt || Date.now() // Ensure joinedAt is set if missing
    };
                  ...
                  <div className="grid gap-4">
                      ...
                      >
                          ...
                          <div>
                              ...
                              <div className="text-slate-400 text-sm">Let Nova discover the perfect career based on your interests.</div>
                          </div>
                      </button>
